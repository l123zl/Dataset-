...
def VOC2Yolo(class_num, voc_img_path, voc_xml_path, yolo_txt_save_path, yolo_img_save_path=None):
    ...
    for idx, one_xml in enumerate(xmls):
        ...
        for ob in objects:
            ...
            try:
                bbox_label = class_num[label] - 1
                label_lines.append("{} {} {} {} {}\n".format(bbox_label,bbox_cx,bbox_cy,bbox_w,bbox_h))
        ...
        print("processing: {}/{}".format(idx,all_xmls))


if __name__ == '__main__':
    f = open("TT100K_VOC_classes.json", 'r')
    info_data = json.load(f)
    print(info_data)
    VOC2Yolo(
        class_num=info_data,  # 标签种类
        voc_img_path=r'xmllabel1/images/test/',  # 数据集图片文件夹存储路径
        voc_xml_path=r'xmllabel1/annotations/test/',  # 标签xml文件夹存储路径
        yolo_txt_save_path=r'xmllabel1/labels/test/'  # 将要生成的txt文件夹存储路径
    )
